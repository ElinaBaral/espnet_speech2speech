...
dict_src=data/lang_1char/${train_set}_units_${src_case}.np.txt
dict_tgt=data/lang_1char/${train_set}_units_${tgt_case}.np.txt
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    text2token.py -s 1 -n 1 -l ${nlsyms} data/${train_set}.np/text.${tgt_case} | cut -f 2- -d" " | tr " " "\n" \
    ...
    text2token.py -s 1 -n 1 -l ${nlsyms} data/${train_set}.np/text.${src_case} | cut -f 2- -d" " | tr " " "\n" \
        | sort | uniq | grep -v -e '^\s*$' | awk '{print $0 " " NR+1}' >> ${dict_src}
    wc -l ${dict_src}

    echo "make json files"
    local/data2json.sh --nj 16 --text data/${train_set}.np/text.${tgt_case} --nlsyms ${nlsyms} --skip_utt2spk true \
        data/${train_set} ${dict_tgt} > ${feat_tr_dir}/data.${src_case}_${tgt_case}.json
    local/data2json.sh --text data/${train_dev}.np/text.${tgt_case} --nlsyms ${nlsyms} --skip_utt2spk true \
        data/${train_dev} ${dict_tgt} > ${feat_dt_dir}/data.${src_case}_${tgt_case}.json
    for rtask in ${recog_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}; mkdir -p ${feat_recog_dir}
        local/data2json.sh --text data/${rtask}.np/text.${tgt_case} --nlsyms ${nlsyms}  --skip_utt2spk true \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    (
        ...
        local/score_bleu.sh --case ${tgt_case} --nlsyms ${nlsyms} ${expdir}/${decode_dir} np ${dict_tgt}
